...
class MLP:
    def __init__(self, n_inputs, n_hidden, n_outputs, lr=0.01):
        ...
        # zmienna do rejestrowania historii dopasowania
        self.loss_history = []
        # i jakości klasyfikacji
        self.acc_history = []
    ...
    def train(self, X, y_true, epochs=100):
        for e in range(epochs):
            ...
            # dodanie dopasowania do historii
...
plt.title("Zmiana wartości lunkcji straty podczas uczenia")
plt.xlabel("Krok uczenia")
plt.ylabel("Entropia krzyżowa")  # lub "MSE" w wersji regresyjnej
plt.grid(True)

plt.subplot(1, 2, 2)
plt.plot(model.acc_history)
plt.title("Dokładność klasyfikacji w kolejnych krokach uczenia")
plt.xlabel("Krok uczenia")
